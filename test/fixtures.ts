...
export async function deployFixtures() {
   ...
   */
  const Multicall3Contract = await ethers.getContractFactory<any, Multicall3>(
    "Multicall3",
  );
  const Multicall3Instance = (await Multicall3Contract.connect(
    owner,
   ...
   */
  const MockedERC20Contract = await ethers.getContractFactory<any, MockedERC20>(
    "MockedERC20",
  );
  const MockedERC20Instance = (await MockedERC20Contract.connect(
    owner,
   ...
   * @dev Funding erc20
   */
  await MockedERC20Instance.connect(owner).transfer(
    owner2.address,
   ...
   * @dev Deploy contract
   */
  const PocketChefContract = await ethers.getContractFactory<any, PocketChef>(
    "PocketChef",
  );
  ...
  )) as unknown as unknown as PocketChef;
  ...
  )) as unknown as unknown as PocketRegistry;
  ...
  )) as unknown as unknown as PocketVault;
  ...
  return {
    ...
    MockedERC20: MockedERC20Instance,
    Registry,
    Vault,
    Chef,
    owner,
